<p align="center">
  ...
  <br>
  <img src="https://img.shields.io/badge/Tests-25%2F25_passing-brightgreen?style=for-the-badge" alt="Tests">
  <img src="https://img.shields.io/badge/Coverage-100%25_lines-brightgreen?style=for-the-badge" alt="Coverage">
...
| **Testing** | 10/10 | 25 host-side test files, all passing. **100.0% line coverage** (0 uncovered) enforced as a hard Sonar quality gate. mbedtls fault-injection via linker `--wrap`, FlakyFilePort precise call-count injection, IEC 62304 §5.5.3 LCOV_EXCL annotations on defensive paths |
...
| 35 | **Lifetime ECG record grand-total** | A cumulative device-lifetime record count is kept as a fourth channel (`RECSTAT`) in the permanent `device.ats`. `rotateDailyDb()` folds the completing day's record count into the running total before renaming `sensor.ats` to `YYYYMMDD.ats`, so the grand total survives daily rotation, reboots and card re-reads. Seeded once from the historical day-files on first upgrade; exposed to the display layer as `StorageStats::lifetimeRecords` (uint64). Note: the total lives on the SD card, so replacing the card (the medical device's persistent memory, ~5–10 yr service life) intentionally starts a fresh count | `AtsStorageServiceImpl.cpp`, `ArcanaTsSchema.hpp` |
| 36 | **Animated boot splash over slow SD init** | Mounting a large SD card can take tens of seconds; a black screen reads as a hang. `BootSplash` renders an animated splash on the ST7789 during `initHAL()` so the device visibly "boots" while the card mounts, then hands the panel to the MVVM `MainView`. Pure view-layer, zero service dependency | `view/BootSplash.hpp` |
| 37 | **LCD shows today + lifetime totals** | The main view renders both the current day's record count ("Today", from the live `sensor.ats`) and the device-lifetime grand total ("Total"), alongside the live sample rate. `LcdOutput` carries `lifetime` as uint64; `MainViewModel` splits the count into the two lines with independent dirty flags so each refreshes only when its value changes | `view/main/MainView.hpp`, `view/main/MainViewModel.hpp` |
...
| 19 | Host tests had no infrastructure | Built up from zero starting at commit `9e11332`. Current state: 25 host test files in Tests/, gtest framework, mbedtls fault injection via linker `--wrap`, `FlakyFilePort` for ICipher/IFilePort failure paths, Sonar reports 100.0% line coverage. Fault-injection design: counter-based `_after_n` flags for "succeed N times then fail on call N+1" patterns |
...
| 29 | dual-FAT torn commit left "free but referenced" clusters; whole-volume reconcile was slow | After the dual-FAT layer (#28) landed, a torn commit could leave clusters marked free in the allocation bitmap yet still chained from a file — a latent leak/corruption. A mount-time reconcile now heals these before any allocation. The first cut walked the whole volume, which cost **~85 s** on a card holding months of day-files (98+ files). Narrowed to the only files that can be open-for-write at a power loss — `sensor.ats` and `device.ats` — since rotated day-files are cleanly closed at rotation: `ExFatVolume::reconcileFile(path)` replaced the whole-volume `reconcileBitmap` scan, and `begin()` no longer auto-reconciles. Mount dropped **~85 s → ~2.4 s** with identical safety, unit-covered by `test_dualfat_reconcile`. Hardware-verified: 13.4 M records intact across a power cut, `fail=0`. Commits `08932f2` (heal) + `6c29c4c` (targeted) | `components/SdFat/` (ExFatVolume/ExFatPartition), `AtsStorageServiceImpl.cpp` |
